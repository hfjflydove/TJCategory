...
- (void)showMessageAtTopWithText:(NSString *)text top:(CGFloat)top
{
    if (!text.length) {
        return;
    }
...
- (UIView *)creatLabelWithText:(NSString *)text atBottom:(BOOL)atBottom
{
    ...
    if(kIsLandscape){ // TJNIMInstance.shareInstance.mKeyBoardlsVisible

    }
    aAlert.layer.masksToBounds = YES;
    aAlert.backgroundColor = UIColorFromRGBA(0x000000, 0.75);
...
-(void)getCornerViewWithRadius:(CGFloat)radiu
{
    ...
    if([self.layer.mask.name isEqualToString:@"maskLayer"]){
        for (CALayer *subLayer in self.layer.sublayers) {
            if([subLayer.name isEqualToString:@"maskLayer"] && subLayer.superlayer){
                maskLayer = subLayer;
                break;
            }
        }
