...
+(NSInteger)getExtLocation:(NSString *)picURL{
    ...
    //.jfif
    NSRange jfifRange = [picURL rangeOfString:@".jfif" options:NSBackwardsSearch range:NSMakeRange(0, picURL.length)];
    while (jfifRange.location != NSNotFound) {
        return jfifRange.location;
    }
    return NSNotFound;
}
